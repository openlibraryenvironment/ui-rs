...
const SlnpRespondYes = ({ performAction }) => {
  ...
  const onSubmit = values => {
    return performAction('slnpRespondYes', values, {
      success: 'ui-rs.actions.slnpRespondYes.success',
      error: 'ui-rs.actions.slnpRespondYes.error',
    })
      .then(closeModal);
  };
    ...
    <Form
      ...
      render={({ handleSubmit, invalid, submitting, pristine, form }) => (
        <form onSubmit={handleSubmit}>
          <Modal
            label={<FormattedMessage id="ui-rs.actions.slnpRespondYes" />}
            open={currentModal === 'SlnpRespondYes'}
            onClose={closeModal}
            dismissible
            footer={<Footer disableSubmit={invalid || pristine || submitting} submit={form.submit} />}
          >
            <form onSubmit={handleSubmit}>
              <FormattedMessage id="ui-rs.actions.slnpRespondYes.callNumber" />
              <Row>
                <Col xs={11}>
                  <Field
                    autoFocus
              ...
              <FormattedMessage id="ui-rs.actions.slnpRespondYes.pickLocation" />
              <Row>
                <Col xs={11}>
                  <Field
                    component={Select}
              ...
              {shelvingOptions.length > 0 &&
                <>
                  <FormattedMessage id="ui-rs.actions.slnpRespondYes.pickShelvingLocation" />
                  <Row>
                    <Col xs={11}>
                      <Field
                        component={Select}
...
SlnpRespondYes.propTypes = {
  performAction: PropTypes.func.isRequired,
};

export default SlnpRespondYes;
