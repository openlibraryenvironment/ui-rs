...
const FileSetting = ({ setting, fileType }) => {
  ...
  const upload = async (dropped) => {
    ...
    const uploadResult = await okapiKy.post('rs/fileDefinition/fileUpload', { body: formData }).json();
    await okapiKy.put(`rs/settings/appSettings/${setting.id}`, { json: { value: uploadResult.id } });
    await invalidate();
    setEditing(false);
